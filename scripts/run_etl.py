...
def main() -> None:
    ...
    parser.add_argument(
        "--no-db", action="store_true", help="Skip the PostgreSQL load step."
    )
    parser.add_argument(
        "--default-stay",
        default=None,
    ...
    default_stay = (
        datetime.strptime(args.default_stay, "%Y-%m-%d").date()
        if args.default_stay
        else None
    )
    summary = run_etl_pipeline(
        default_stay_date=default_stay, persist_to_db=not args.no_db
    )
